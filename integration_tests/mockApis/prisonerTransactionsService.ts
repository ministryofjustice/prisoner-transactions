...
const stubRequestLink = (): SuperAgentRequest =>
  stubFor({
    request: {
      ...
      bodyPatterns: [
        { matchesJsonPath: "$[?(@.email == 'mike.halma@digital.justice.gov.uk' && @.sessionID =~ /.*/i)]" },
      ],
    },
    response: {
      status: 200,
...
const stubCreateBarcode = (): SuperAgentRequest =>
  stubFor({
    request: {
      method: 'POST',
      urlPattern: '/prisoner-transactions/barcode',
      headers: {
      ...
      bodyPatterns: [{ equalToJson: `{ "userId": "mike.halma@digital.justice.gov.uk", "prisonerId": "A1234BC" }` }],
    },
    response: {
      status: 200,
      headers: {
